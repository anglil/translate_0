...
def get_seed_for_data_selection3(dev_ref, test_1best, proc_seed_for_lm_training_selection):
    ...
    if not os.path.exists(seed_for_lm_training_selection) or os.stat(seed_for_lm_training_selection).st_size==0:
        merge_files([dev_ref, test_1best], seed_for_lm_training_selection)
    ...
    if os.listdir(selected_non_domain_doc_dir) == []:
        ...
        with open(proc_seed_in_domain) as f:
            for line in f:
                l = line.strip().split(' ')
                for tok in l:
                    in_domain_tok.add(tok.lower()) # lower case
            ...
            for line in fp:
                ...
                if line.strip() != "=":
                    ...
                    for tok in l:
                        ...
                        if tok.lower() in in_domain_tok:
                            intersection_wordcount += 1 # lower case
                else:
...
def select_doc_with_high_jaccard_idx3(
    proc_seed_in_domain,
    doc_non_domain,
    doc_non_domain_proc,
    ...
    if not os.path.exists(selected_non_domain_doc_with_unk_file):
        ...
        # rank documents by jaccard index
        with open(doc_non_domain) as fnp, open(doc_non_domain_proc) as fp:
            for line in fp:
                line_np = fnp.readline()
...
# obsolete
def select_doc_with_high_jaccard_idx2(
    proc_seed_in_domain, 
    doc_non_domain, 
    doc_non_domain_proc, 
    num_of_doc_to_select, 
